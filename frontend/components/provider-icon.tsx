// Provider brand logos as inline SVGs
// Sources: Simple Icons (CC0), LobeHub lobe-icons (MIT), official brand kits
// No external loading — instant render, dark-mode compatible

import type { AIProviderType } from "@openclaw-business/shared";
...
export function ProviderIcon({ provider, size = 24, className = "" }: ProviderIconProps) {
  ...
  switch (key) {
    ...
    // ── xAI (Grok) — LobeHub lobe-icons (MIT) ──────────────────────
    case "xai":
      return (
    ...
    // ── Groq — LobeHub lobe-icons (MIT) ─────────────────────────────
    case "groq":
      return (
        <svg width={size} height={size} viewBox="0 0 24 24" fill="#F55036" className={className}>
    ...
    // ── Custom provider — code brackets ─────────────────────────────
    case "custom":
      return (
        <svg width={size} height={size} viewBox="0 0 24 24" fill="none" className={className}>
          <rect x="2" y="2" width="20" height="20" rx="4" stroke="currentColor" strokeWidth="1.5" opacity="0.5" />
          <path d="M9.5 8L6.5 12L9.5 16M14.5 8L17.5 12L14.5 16" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
        </svg>
      );

    // ── Fallback — two-letter abbreviation ──────────────────────────
    default:
      return (
        <svg width={size} height={size} viewBox="0 0 24 24" fill="none" className={className}>
